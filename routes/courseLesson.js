...
router.put("/admin/lesson/update/:id", Auth, async (req, res, next) => {
    ...
    try {
        const lessonUpdate = await CourseLesson.findByIdAndUpdate({ _id: id, },
            { $set: { name, lesson: file, description, } })
        console.log(lessonUpdate)
        return res.status(200).json({ msg: "kurs vuvafaqiyatli o'zgartirildi" })
    } catch (error) {
...
router.put("/admin/coursBooks/update/:id",Auth, async (req, res) => {
    ...
    const coursBooks = req.files?.books
    try {
        const file = coursBooks && uuid.v4() + "-file" + ".jpg"
        coursBooks?.mv(path.resolve(__dirname, "..", "CourseFile", file))
